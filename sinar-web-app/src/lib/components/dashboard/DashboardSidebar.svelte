<script lang="ts">
  ...
  const menuSections = [
    {
      title: "Main",
      roles: ["admin"], // Only available for admin users
      items: [
        {
...
>
  ...
  <div class="p-6 border-b border-gray-200/50 relative">
    <div
      class="flex items-center {isCollapsed ? 'justify-center' : 'space-x-3'}"
    >
  ...
  <nav class="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
    {#each visibleMenuSections() as section}
      <div>
        ...
        {#if !isCollapsed}
          <h4
            class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 px-3 transition-opacity duration-300"
          >
            {section.title}
          </h4>
        {:else}
          ...
          {#each section.items as item}
            <a
              ...
              class="flex items-center {isCollapsed
                ? 'justify-center px-2'
                : 'space-x-3 px-3'} py-2 rounded-lg text-sm font-medium transition-all duration-200 relative group
  ...
  <div class="p-4 border-t border-gray-200/50">
    <div
      class="text-xs text-gray-500 {isCollapsed
        ? 'text-center'
        : 'text-center'}"
    >
      {#if !isCollapsed}
