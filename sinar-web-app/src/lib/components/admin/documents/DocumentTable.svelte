<script lang="ts">
  ...
  // Fetch paginated documents from API
  async function fetchDocuments() {
    console.log("Starting fetchDocuments with pagination...");
    isLoading = true;
    error = null;

    try {
      console.log("Calling documentService.getPaginatedDocuments()...");
  ...
  // Server-side pagination - data is already filtered and paginated from backend
  ...
  function handleSearchChange(event: Event) {
    ...
    currentPage = 1; // Reset to first page when searching
    fetchDocuments(); // Fetch data with new search term
  }

  function handlePageChange(newPage: number) {
    if (newPage >= 1 && newPage <= totalPages()) {
      currentPage = newPage;
      fetchDocuments(); // Fetch data for new page
    }
  }
  ...
  {:else}
    ...
    <div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-200 bg-white sticky bottom-0 z-10">
      <div class="text-sm text-gray-700">
        Showing <span class="font-medium">{Math.min((currentPage - 1) * pageSize + 1, totalRecords)}</span>
        to
