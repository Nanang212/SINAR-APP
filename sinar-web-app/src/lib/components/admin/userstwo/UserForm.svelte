<script lang="ts">
  ...
  //   }
  // }
  async function loadUserProfilePhoto(userId: number, version?: string) {
    try {
      // Clean up blob lama
      if (logoPreview && logoPreview.startsWith("blob:")) {
        userService.revokeProfilePhotoUrl(logoPreview);
      ...
      // Inject cache-buster sekali (stabil)
      const userIdWithQs = version
        ? `${userId}?v=${encodeURIComponent(version)}`
        : `${userId}`;

      // service kamu menerima string | number → aman di-cast
      const photoUrl = await userService.getUserProfilePhotoUrl(
        userIdWithQs as unknown as number
      );

      if (photoUrl) {
        logoPreview = photoUrl; // blob:... (bukan base64) → cukup buat form preview
      }
    } catch (error) {
      console.error("Failed to load profile photo:", error);
    }
  ...
  function populateForm(user: User) {
    ...
    console.log(
      "🔍 populateForm comparison: loadedUserId =",
      loadedUserId,
      "user.id =",
      user.id
    );
    if (loadedUserId !== user.id) {
      console.log(
        "🔄 populateForm: Different user, resetting hasCompletedUpdate"
      );
      hasCompletedUpdate = false;
    } else {
      console.log(
        "📌 populateForm: Same user, keeping hasCompletedUpdate =",
        hasCompletedUpdate
      );
    }

    console.log(
      "📝 populateForm called - hasCompletedUpdate:",
      hasCompletedUpdate,
      "isFormDisabled:",
      isFormDisabled
    );
    ...
    (form.querySelector("#username") as HTMLInputElement).value =
      user.username || "";
    (form.querySelector("#contact_person") as HTMLInputElement).value =
      user.contact_person || "";
    (form.querySelector("#name_mentri") as HTMLInputElement).value =
      user.name_mentri || "";
    ...
    if (user.logo_url && user.id && loadedUserId !== user.id) {
      loadedUserId = user.id;
      // kalau ada updated_at, kirim sebagai versi supaya cache busting otomatis
      loadUserProfilePhoto(user.id, user.updated_at || Date.now());
    }
  }
    ...
    try {
      if (isEditMode && userData) {
        ...
        const updateData: UpdateUserRequest = {
          ...
          contact_person:
            (formData.get("contact_person") as string) || undefined,
        ...
        //   console.log(
        //     "🎯 Set hasCompletedUpdate = true, isFormDisabled =",
        //     isFormDisabled
        //   );
        // }
        if (result.status) {
          ...
          // Ambil foto terbaru dari server SATU kali, pakai updated_at baru
          const newVersion = result.data?.updated_at || Date.now().toString();
          await loadUserProfilePhoto(userData.id, newVersion);

          // Bersihkan state file lokal (opsional)
  ...
  function handleReset() {
    console.log(
      "🧹 handleReset called, hasCompletedUpdate was:",
      hasCompletedUpdate
    );
  ...
  function applyCrop() {
    ...
    try {
      ...
      if (typeof cropper.getCroppedCanvas !== "function") {
        console.error(
          "❌ Cropper object does not have getCroppedCanvas method:",
          cropper
        );
    ...
    <form bind:this={formRef} class="space-y-6" onsubmit={handleSubmit}>
        ...
        <div class="relative">
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <svg
                class="w-4 h-4 text-gray-400 transition-transform duration-200 {isRoleDropdownOpen
                  ? 'rotate-180'
                  : ''}"
                fill="none"
                stroke="currentColor"
          ...
          {#if isRoleDropdownOpen}
            <div
              class="absolute z-50 w-full mt-1 bg-white border-2 border-gray-200 rounded-lg shadow-lg overflow-hidden"
            >
              {#each roles as role}
                <button
                  type="button"
                  onclick={() => handleRoleSelect(role.id.toString())}
                  class="w-full px-3 py-2.5 text-left hover:bg-blue-50 focus:bg-blue-50 focus:outline-none transition-colors duration-150 border-b border-gray-100 last:border-b-0 {selectedRole ===
                  role.id.toString()
      ...
      {#if selectedRole === "2"}
        <div class="relative">
          ...
          {#if categoriesLoading}
            <div
              class="flex items-center space-x-3 p-4 border-2 border-gray-200 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 shadow-sm"
            >
              <svg
            ...
            <div class="relative">
              ...
              >
                <div class="flex items-center justify-between">
                  ...
                  <svg
                    class="w-4 h-4 text-gray-400 transition-transform duration-200 {isCategoryDropdownOpen
                      ? 'rotate-180'
                      : ''}"
                    fill="none"
                    stroke="currentColor"
              ...
              {#if isCategoryDropdownOpen}
                <div
                  class="absolute z-50 w-full mt-1 bg-white border-2 border-gray-200 rounded-lg shadow-lg overflow-hidden max-h-48 overflow-y-auto"
                >
                  <!-- None option -->
                  <button
                    type="button"
                    onclick={() => handleCategorySelect("")}
                    class="w-full px-3 py-2.5 text-left hover:bg-green-50 focus:bg-green-50 focus:outline-none transition-colors duration-150 border-b border-gray-100 {selectedCategory ===
                    ''
                  ...
                  {#each categories as category}
                    <button
                      ...
                      class="w-full px-3 py-2.5 text-left hover:bg-green-50 focus:bg-green-50 focus:outline-none transition-colors duration-150 border-b border-gray-100 last:border-b-0 {selectedCategory ===
      ...
      <div>
        <label for="logo" class="block text-sm font-medium text-gray-700 mb-3">
          Profile Photo {isEditMode
            ? "(Optional - leave empty to keep current photo)"
            : "(Optional)"}
        </label>
        ...
        {:else}
          ...
          <div
            role="button"
            tabindex="0"
            class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors {isFormDisabled
          ...
          >
            <div class="flex flex-col items-center">
              ...
              <p class="text-xs text-gray-400">
                PNG, JPG files up to 5MB • Will be cropped to perfect circle
              </p>
            </div>
          </div>
        {/if}
      ...
      <div class="flex items-center justify-end space-x-4 pt-4">
        <button
          ...
          class="px-6 py-3 text-base font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed {isFormDisabled &&
          !isSubmitting
            ? 'ring-2 ring-green-500 ring-opacity-50'
            : ''}"
          title={isFormDisabled && !isSubmitting
            ? "Click to reset and re-enable form"
            : ""}
        >
          Reset
        </button>
          ...
          >
            ...
            >
              <path
                ...
                stroke-linejoin="round"
                stroke-width="2"
                d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 12H9v4a1 1 0 01-1 1H4a1 1 0 01-1-1v-4c0-2.519 1.981-4.5 4.5-4.5s4.5 1.981 4.5 4.5z"
              />
            </svg>
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">
            Adjust the crop area to get the perfect profile photo
          </p>
        </div>
        <button
      ...
      <div class="flex-1 p-6 overflow-hidden">
        <div class="flex gap-6 h-full">
          ...
          >
            ...
            {#if cropper}
              <div
                class="absolute bottom-4 right-4 flex space-x-2 bg-black bg-opacity-50 rounded p-2"
              >
                <button
                  type="button"
          ...
          <div class="w-64 flex flex-col items-center">
            ...
            <!-- Circle Preview -->
            <div
              class="w-32 h-32 rounded-full border-4 border-gray-200 shadow-md bg-gray-100 mb-4 overflow-hidden relative"
            >
              {#if cropper}
                <canvas id="preview-canvas" class="w-full h-full object-cover"
                ></canvas>
              {:else}
                <div
...
{#if showPasswordModal}
  ...
  >
    <div
      class="bg-white rounded-lg shadow-2xl max-w-md w-full mx-4 border border-gray-200"
    >
      <!-- Header -->
      <div
        class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-red-50"
      >
        <div class="flex items-center justify-between">
      ...
      <div class="px-6 py-6">
        <p class="text-gray-600 text-sm mb-6">
          Reset password for user: <span class="font-medium text-gray-900"
      ...
      <!-- Actions -->
      <div
        class="px-6 py-4 bg-gray-50 flex flex-col sm:flex-row gap-3 sm:gap-0 sm:justify-end sm:space-x-3"
      >
        <button
          onclick={closePasswordModal}
          disabled={isResettingPassword}
