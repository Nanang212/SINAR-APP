<script lang="ts">
  ...
  function transformApiDocument(docGroup: any): DisplayDocument {
    ...
    ["TEXT", "LINK", "AUDIO", "VIDEO"].forEach((type) => {
      if (reports[type] && reports[type].length > 0) {
        const latestInType = reports[type].reduce(
        ...
        if (
          !latestReport ||
          new Date(latestInType.updated_at || latestInType.created_at) >
            new Date(latestReport.updated_at || latestReport.created_at)
        ) {
  ...
  async function fetchReports(
    ...
    search: string = "",
    order: "asc" | "desc" = "desc",
    start: string | null = startDate,
    end: string | null = endDate
  ) {
    console.log(
      `Starting fetchReports with pagination - Page: ${page}, Search: "${search}", Order: ${order}`
    );
    isLoading = true;
    error = null;
    ...
    try {
      ...
      console.log(
        "Calling reportService.getPaginatedReports() with backend pagination, search and sort..."
      );
      ...
      if (response.status === true && response.data) {
        ...
        if (paginatedData.data && Array.isArray(paginatedData.data)) {
          ...
          console.error(
            "Invalid response format - expected nested data with data array:",
            paginatedData
          );
  ...
  async function handleRefresh() {
    await fetchReports(currentPage, searchTerm, sortOrder, startDate, endDate);
    onRefresh?.();
  }
  ...
  onMount(() => {
    if (fetchOnMount) {
      fetchReports(1, searchTerm, sortOrder, startDate, endDate);
    }
    ...
    const handleWindowFocus = async () => {
      if (pendingDownloadRefresh) {
        console.log(
          "Window regained focus, refreshing report data after download..."
        );
  ...
  export function loadReports() {
    return fetchReports(1, searchTerm, sortOrder, startDate, endDate);
  }
  ...
  export function setSearchParams(search: string, order: "asc" | "desc") {
    ...
    fetchReports(1, search, order, startDate, endDate);
  }

  async function handlePageChange(newPage: number) {
    if (newPage >= 1 && newPage <= filteredTotalPages()) {
      await fetchReports(newPage, searchTerm, sortOrder, startDate, endDate);
    }
  }
  ...
  function handleRowClick(document: DisplayDocument) {
    ...
    const apiDocument = apiDocuments.find(
      (apiDoc) => apiDoc.document.id.toString() === document.id
    );
  ...
  function formatMediaCounts(document: DisplayDocument): string {
    ...
    if (document.textCount > 0) {
      counts.push(
        `${document.textCount} Text${document.textCount > 1 ? "s" : ""}`
      );
    }
    if (document.linkCount > 0) {
      counts.push(
        `${document.linkCount} Link${document.linkCount > 1 ? "s" : ""}`
      );
    }
    if (document.audioCount > 0) {
      counts.push(
        `${document.audioCount} Audio${document.audioCount > 1 ? "s" : ""}`
      );
    }
    if (document.videoCount > 0) {
      counts.push(
        `${document.videoCount} Video${document.videoCount > 1 ? "s" : ""}`
      );
    }
